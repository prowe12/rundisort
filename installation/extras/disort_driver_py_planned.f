...
c     Lines added for f2py:
      intent(in)  :: NLYR, DTAUC, SSALB_CLD, NMOM,
      intent(in)  :: NCLDLYR, CLDLYR, PMOM_CLD,
      intent(in)  :: TEMPER, WVNMLO, WVNMHI, USRTAU, NTAU, UTAU, NSTR,
      intent(in)  :: USRANG, NUMU, UMU, NPHI, PHI, IBCND, FBEAM,
      intent(in)  :: UMU0, PHI0,FISOT, LAMBER, ALBEDO, BTEMP,
      intent(in)  :: TTEMP,zTEMIS,PLANK, ONLYFL, HEADER,
      intent(in)  :: MAXCLY, MAXULV, MAXUMU, MAXPHI, MAXMOM

      intent(out) :: RFLDIR,RFLDN,FLUP,DFDT,UAVG,UU,ALBMED,TRNMED




	 
c    .. Initialize PMOM and SSALB matrices to zeros
      DO J=1,MAXCLY
        SSALB(J) = 0.0
        DO I=0,MAXMOM
          PMOM(I,J) = 0.0
...
c   ..Fill in the SSALB and moments from the namelist
      DO ICLDLYR=1,NCLDLYR
        SSALB(CLDLYR(ICLDLYR)) = SSALB_CLD(ICLDLYR)
        DO I=0,NMOM
